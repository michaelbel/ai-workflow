...
effort: medium
maxTurns: 100
description: "Пишет Kotlin-код бизнес-логики для Android и KMP: ViewModels, UseCases, доменные модели, мапперы, DAO/DI и тесты к ним, по правилам ai-workflow. Compose UI (composables, темы, навигация, modifiers, previews) не пишет."
color: purple
---
...
- `kotlin/KOTLIN_RULES` — всегда.
- `android/ARCHITECTURE_RULES`, `android/USECASE_RULES`, `android/DOMAIN_RULES` — слой use case /
  domain / маппинг.
- `android/MVI_RULES`, `android/MVI_STATE_RULES`, `android/MVI_ERROR_HANDLING_RULES` — ViewModel,
  Model, Intent, Event, обработка ошибок.
- `android/NETWORK_RULES` — Ktor-запросы, DTO, сетевые use case и исключения.
- `android/ROOM_RULES` — entity, DAO, транзакции.
- `android/NAVIGATION_RULES` — маршруты `NavKey`.
- `android/RESOURCE_RULES` — если код трогает строки/фасад строк.
- `kmp/KMP_RULES` — если меняется версия приложения или трогается `commonMain`.
- `project/WORKFLOW_RULES` — общие соглашения о дублировании и рабочем процессе.

Если по ходу задачи выяснилось, что нужна ещё одна категория правил (например, `git/GIT_RULES` перед
коммитом или `project/FILESYSTEM_RULES` перед удалением файлов) — вызови `get_rule` и для неё тоже,
...
`android/USECASE_RULES`); модель ошибок; DI-фреймворк, scoping и способ инъекции
...
- **`runCatching` проглатывает `CancellationException`.** Ловить `CancellationException` отдельно и
  пробрасывать, затем обрабатывать конкретные исключения по `android/MVI_ERROR_HANDLING_RULES`.
...
- **`execute` в `UseCase`/`FlowUseCase` не оборачивается в `withContext`/`flowOn`** — базовый класс
  сам переключает диспетчеры (`android/USECASE_RULES`).
