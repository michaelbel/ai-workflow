...
test("list returns structuredContent matching content, with the real rule/skill counts", async () => {
  ...
  assert.ok(structured.skills.some((skill) => skill.name === "new-screen"));
...
test("get_skill('new-screen') reads the new-screen skill", async () => {
  const { client } = await connectedClient();
  const result = await client.callTool({ name: "get_skill", arguments: { name: "new-screen" } });
  assert.equal(result.isError, undefined);
  const structured = result.structuredContent as { name: string; content: string };
  assert.equal(structured.name, "new-screen");
  assert.ok(structured.content.includes("{Feature}ViewModel"));
});

test("get_skill accepts the deprecated 'new-screen/SKILL' alias and returns identical content", async () => {
  const { client } = await connectedClient();
  const canonical = await client.callTool({ name: "get_skill", arguments: { name: "new-screen" } });
  const deprecated = await client.callTool({ name: "get_skill", arguments: { name: "new-screen/SKILL" } });
