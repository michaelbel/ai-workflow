...
- In use case network calls, always create request objects in a separate local `val request` before calling `networkService`; do not inline request construction or mapped request calls inside `networkService` arguments.
