...
- Вспомогательные значения, которые использует только `PreviewParameterProvider` (списки сущностей,
  отдельные экземпляры), объявляй приватными свойствами внутри самого класса провайдера перед
  `values`, а не на уровне файла.
