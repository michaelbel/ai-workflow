---
description: >-
  Длина строки до 150 символов, when вместо цепочек if/else if, return when, fail-fast ранним
  return/throw вместо вложенных if/else, однострочные ветки when только при полном размещении
paths:
  - "**/*.gradle.kts"
  - "**/*.kt"
---
...
- При возврате одной из двух веток предпочитай `return when { ... }` вместо
  `return if (...) ... else ...`.
- Применяй принцип fail-fast: отсеивай невалидные и граничные случаи ранним `return`, `throw` или
  `require` / `check` в начале функции, а не оборачивай основную логику во вложенные `if / else`.
